...
Create nested group paths such as `Research/Imaging`. A paper may belong to several groups. In the center list, click a row to select it or use Ctrl/Command-click to select several. In the always-visible **Assign selected papers** section on the left, choose the target group and click **Add to group** or **Remove from group**. Clicking a group in the group tree filters the list; it does not assign papers. To delete the selected group and all groups nested below it, click **Delete selected group** and confirm. Group assignments are removed, but paper records and PDFs are not deleted.
