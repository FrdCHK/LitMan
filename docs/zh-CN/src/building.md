...
```powershell
./scripts/package-windows.ps1 -Version 0.1.7
```

脚本会构建两个程序和两种手册，并生成 `dist/LitMan-0.1.7-x64.msi`、单文件 `dist/LitMan-0.1.7-portable-x64.exe`，以及包含 `LitMan.exe`、`litman-cli.exe`、许可证和离线手册的便携版 ZIP。MSI 包含 GUI、CLI、开始菜单快捷方式、本地手册、卸载信息，以及可选的 CLI PATH 组件。设置 `LITMAN_CERT_THUMBPRINT` 并传入 `-Sign` 可调用 `signtool`；时间戳地址由 `LITMAN_TIMESTAMP_URL` 控制。
...
./scripts/package-macos.sh 0.1.7
```
...
```console
./scripts/package-deb.sh 0.1.7
sudo apt install ./dist/litman_0.1.7_amd64.deb
```
